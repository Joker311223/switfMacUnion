...
`docs/` 目录下每款应用对应一个独立 HTML 页面，`index.html` 是合集汇总首页。按以下规则更新：

| 改动类型 | 需要更新的文件 | 具体操作 |
|----------|----------------|----------|
| 新增应用 | `docs/xxx.html` + `docs/index.html` | 新建 `docs/应用名小写.html`（参照现有详情页结构）；在 `index.html` 的应用卡片网格中追加对应卡片，并将跳转链接设置为 `https://joker311223.github.io/switfMacUnion/应用名小写` |
| 新增功能 | `docs/xxx.html` | 仅修改对应应用的详情页，在「核心功能」区块追加功能卡片 |
| 新增快捷键 | `docs/xxx.html` | 仅修改对应应用的详情页，在快捷键表格中追加一行 |
| 新增偏好设置项 | `docs/xxx.html` | 仅修改对应应用的详情页，在偏好设置对应 Tab 的列表中追加描述 |
| 删除应用 | `docs/xxx.html` + `docs/index.html` | 删除对应详情页；同步删除 `index.html` 中的应用卡片 |
| 删除功能 | `docs/xxx.html` | 仅修改对应应用的详情页，删除对应功能卡片 |

> **页面跳转规则**：详情页 URL 格式统一为 `https://joker311223.github.io/switfMacUnion/应用名小写`（不含 `.html` 后缀），`index.html` 中所有应用卡片的 `href` 必须遵循此格式。
